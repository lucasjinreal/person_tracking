...
KidonoHumanClassifier::KidonoHumanClassifier(const std::string &modelfile,
                                             const std::string &scalefile)
    : scale(scalefile, 213) {
...
#else
  cv::Ptr<Boost> boost = StatModel::load<Boost>(modelfile);
...
bool KidonoHumanClassifier::predict(
