...
// TODO: change all codes related to CloudCluster into protobuf?
// however is not the best option

namespace kkl {
namespace alg {
template<>
boost::optional<double> distance(const std::shared_ptr<hdl_people_tracking::KalmanTracker> &tracker,
                                 const cti_msgs::CloudCluster &observation) {
  Eigen::Vector3d
      pos(observation.centroid_point.point.x, observation.centroid_point.point.y, observation.centroid_point.point.z);
...
class PeopleTracker {
 public:
  PeopleTracker(ros::NodeHandle &private_nh) {
    ...
    data_association.reset(new kkl::alg::NearestNeighborAssociation<KalmanTracker::Ptr, cti_msgs::CloudCluster>());
//    data_association.reset(new kkl::alg::GlobalNearestNeighborAssociation<KalmanTracker::Ptr, VisualDetection>());
   ...
   */
  void correct(const ros::Time &time, const std::vector<cti_msgs::CloudCluster> &detections) {
    // data association
    ...
    for (const auto &assoc : associations) {
      ...
      const auto &observation = detections[assoc.observation].centroid_point.point;
    ...
    for (int i = 0; i < detections.size(); i++) {
      if (!associated[i]) {
        ...
        const auto &observation = detections[i].centroid_point.point;
        Eigen::Vector3d observation_pos(observation.x, observation.y, observation.z);
 ...
 public:
  ...
  std::unique_ptr<kkl::alg::DataAssociation<KalmanTracker::Ptr, cti_msgs::CloudCluster>> data_association;
};
