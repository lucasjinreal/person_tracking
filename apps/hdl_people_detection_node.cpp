...
class HdlPeopleDetectionNodelet {
 public:
  ...
  HdlPeopleDetectionNodelet() {
    ...
    // publishers
    backsub_points_pub =
        private_nh.advertise<sensor_msgs::PointCloud2>("backsub_points", 5);
    cluster_points_pub =
        private_nh.advertise<sensor_msgs::PointCloud2>("cluster_points", 5);
    human_points_pub =
        private_nh.advertise<sensor_msgs::PointCloud2>("human_points", 5);

    detection_markers_pub =
        private_nh.advertise<visualization_msgs::MarkerArray>(
            "detection_markers", 5);
    backsub_voxel_points_pub = private_nh.advertise<sensor_msgs::PointCloud2>(
        "backsub_voxel_points", 1, true);
    backsub_voxel_markers_pub =
        private_nh.advertise<visualization_msgs::Marker>("backsub_voxel_marker",
                                                         1, true);
    clusters_pub =
        private_nh.advertise<hdl_people_tracking::ClusterArray>("clusters", 10);

    // subscribers
    globalmap_sub = nh.subscribe(
        "/globalmap", 1, &HdlPeopleDetectionNodelet::globalmap_callback, this);
    if (private_nh.param<bool>("static_sensor", false)) {
      static_points_sub =
          mt_nh.subscribe("/velodyne_points", 32,
                          &HdlPeopleDetectionNodelet::callback_static, this);
    } else {
      odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(
          mt_nh, "/odom", 20));
      points_sub.reset(
          new message_filters::Subscriber<sensor_msgs::PointCloud2>(
              mt_nh, "/velodyne_points", 20));
      sync.reset(
          new message_filters::TimeSynchronizer<nav_msgs::Odometry,
                                                sensor_msgs::PointCloud2>(
              *odom_sub, *points_sub, 20));
 ...
 private:
  void publish_msgs(const ros::Time& stamp,
    ...
    if (clusters_pub.getNumSubscribers()) {
      hdl_people_tracking::ClusterArrayPtr clusters_msg(
          new hdl_people_tracking::ClusterArray());
 ...
 private:
  ...
  std::unique_ptr<message_filters::Subscriber<sensor_msgs::PointCloud2>>
      points_sub;
  std::unique_ptr<message_filters::TimeSynchronizer<nav_msgs::Odometry,
                                                    sensor_msgs::PointCloud2>>
      sync;
...
int main(int argc, char** argv) {
  ...
  HdlPeopleDetectionNodelet *personDetNode = new HdlPeopleDetectionNodelet();
